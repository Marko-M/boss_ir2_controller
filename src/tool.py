...
class BossIR2Manager(BossIR2):
    def send_rq1(self, addr, size=1):
        ...
        # Address + Size
        # Size uses 4 bytes. Assume a small size (<128) for now.
    ...
    def read_param(self, name, timeout=1.0):
        ...
        # Open the input before sending the request so a fast response is not missed.
    ...
    def dump_preset(self, verbose=True):
        """Read all known parameters from the pedal."""
        if verbose:
        ...
        for name in PARAMETERS:
            ...
            if val is not None:
                ...
                if PARAMETERS[name]['type'] == 'enum':
                    ...
                    if 0 <= val < len(values):
                        ...
                        if verbose:
                            print(f"  ❌ Invalid enum value for {name}: {val}")
                else:
                    if verbose:
                        print(f"  • {name}: {val}")
            ...
            time.sleep(0.15)  # Delay between commands. Was 0.05.
...
if __name__ == "__main__":
    try:
        ...
        while True:
            ...
            if choice == "1":
                preset = manager.dump_preset(verbose=True)
